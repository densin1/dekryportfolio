...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ============================================================
  // МОВА: поточна мова рендеру контенту (керується i18n.js)
  // ============================================================
  let currentLang = (typeof window.DK_LANG === 'string') ? window.DK_LANG : 'uk';
  ...
  function renderVideoFilter() {
    ...
    const categories = [
      { key: 'all', label: C().ui.filterAll },
      { key: 'reels', label: C().ui.filterReels },
      { key: 'youtube', label: C().ui.filterYoutube }
    ];
    categories.forEach(cat => {
      const btn = document.createElement('button');
      btn.className = 'filter__btn' + (cat.key === activeFilter ? ' is-active' : '');
  ...
  function renderDesignGrid() {
    ...
    C().designProjects.forEach((proj, index) => {
      const item = document.createElement('article');
  ...
  function renderTestimonials() {
    ...
    C().testimonials.forEach((t, i) => {
      ...
      const ratingAria = ui.ratingAriaTemplate.replace('{rating}', t.rating);
      slide.innerHTML = `
        <article class="tg-bubble reveal">
          <header class="tg-bubble__header">
            ...
            <div class="tg-bubble__stars" aria-label="${ratingAria}">${'★'.repeat(t.rating)}</div>
          </header>
    ...
    testimDots = [...testimonialsDots.children];

    if (testimCount > 0) {
      goToTestimonial(0);
      startTestimAutoplay();
    }
    const showArrows = testimCount > 1;
    testimPrev.style.display = showArrows ? '' : 'none';
    testimNext.style.display = showArrows ? '' : 'none';
    testimonialsDots.style.display = showArrows ? '' : 'none';
  ...
  function renderPricing() {
    ...
    C().pricing.forEach(p => {
      ...
      item.innerHTML = `
        ${p.popular ? `<div class="pricing-card__badge">${ui.popularBadge}</div>` : ''}
        <h3 class="pricing-card__name">${p.name}</h3>
        <div class="pricing-card__price">${p.price}</div>
        <ul class="pricing-card__features">
          ${p.features.map(f => `<li>${f}</li>`).join('')}
        </ul>
        <a href="https://t.me/des1n" target="_blank" rel="noopener" class="btn ${p.popular ? 'btn--primary' : 'btn--outline'} btn--block">${ui.ctaDiscussTelegram}</a>
      `;
  ...
  // ============================================================
  // РЕНДЕР УСЬОГО КОНТЕНТУ ДЛЯ ПОТОЧНОЇ МОВИ
  // ============================================================
  function renderAll() {
    renderNav();
    renderHero();
  ...
  // Викликається з i18n.js при зміні мови
