...
function dkBuildLangSwitcher() {
  ...
  slot.innerHTML = `
    <button class="lang-switch__btn" id="langSwitchBtn" type="button" aria-haspopup="true" aria-expanded="false">
      <span id="langSwitchCurrent">${DK_LANGS.find(l => l.code === DK_LANG).label}</span>
  ...
  dkUpdateSwitcherUI(DK_LANG);
}

document.addEventListener('DOMContentLoaded', () => {
  DK_LANG = dkDetectInitialLang();
  dkApplyStaticTranslations(DK_LANG);
  dkBuildLangSwitcher();
});
